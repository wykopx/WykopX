...
(async function ()
{
	...
	async function sprawdzLink(link)
	{
		try
		{
	...
	function losujLink(rodzaj)
	{
		var maksymalnaLiczba;
		var prefix;

		if (rodzaj === "wpis")
		{
			maksymalnaLiczba = 74644353;
			prefix = "https://wykop.pl/wpis/";
		}
		else if (rodzaj === "znalezisko")
		{
			maksymalnaLiczba = 7337723;
			prefix = "https://wykop.pl/link/";
		}
		else
		{
			return null;
		...
		var losowaLiczba = Math.floor(Math.random() * (maksymalnaLiczba / 2)) * 2 + 1;
		return prefix + losowaLiczba;
	}

	async function otworzLink(link)
	{
		window.location.href = link;
	}

	async function otworzLosowyLink(rodzaj)
	{
		var link = losujLink(rodzaj);

		while (!(await sprawdzLink(link)))
		{
			console.log("Link " + link + " nie istnieje. Generowanie nowego...");
			link = losujLink(rodzaj);
		}

		console.log("Otwieranie linku: " + link);
		otworzLink(link);
	}
	...
	if (xhook != null && (settings.wxsBlockXHRExternal || settings.wxsBlockXHRInternalAds))
	{
		...
		xhook.before((request, callback) =>
		{
			...
			{
				if (dev) console.log("Wykop XS - XHR Blocker | XHR: 🌍 " + request.url);
				callback();
			}
			else
			{
